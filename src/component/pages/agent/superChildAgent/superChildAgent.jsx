...
import { SystemPromtPostApi } from '../../../../Utils/Apis';
...
.page-wrapper{
  min-height:28%;      /* full screen height */
  display: flex;
...
const superChildAgent = () => {
    ...
    const goalPromt = localStorage.getItem('GoalPrompt')
    console.log('value id for -----', goalPromt)
    const [loaderCheck, setLoaderCheck] = useState(false);
    const [systemPromptData, setSystemPromptDatak] = useState('');

    const [select, setSelect] = useState('Additional voice')

    // system pormpt api 
    const SystemPromtApi = async () => {

        setLoaderCheck(true)
        try {
            const response = await SystemPromtPostApi(goalPromt);
            console.log('value SystemPromtApi', response)
            if (response?.data?.status === "success") {
                toast.success(response?.data?.message);
                setSystemPromptDatak(response?.data?.systemPrompt)
                ...
                <div className="row">
                    <div className="col-lg-7 col-md-12 col-sm-12 mb-3">
                        <div>
                            ...
                            </span> {'('} <span onClick={(e) => SystemPromtApi()} style={{ cursor: 'pointer' }}><a href="">
                            ...
                            </a></span> <span style={{ color: "#5041BC", cursor: 'pointer' }} onClick={(e) => SystemPromtApi()}>Generate with AI {')'}</span></label>
                            <div className='scrollableData'>
                                <p>{systemPromptData}</p>
                            </div>
                            ...
                            <div className="bottom-btn-wrapper">
                                <div className="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-end">
                                    <Link to="/agent">
                                        <button
                                            type="button"
                                            className="finish-btn btn btn-md me-1"
                                        >
                                            Finish
                                        </button>
                                    </Link>
